...
def answer_association_question(question):
    """
    Répond aux questions concernant les associations étudiantes
    en renvoyant une liste catégorisée des associations du campus.
    SEULEMENT si c'est une demande explicite de liste d'associations.
    """
    # Vérifier si c'est une question concernant les associations
    question_lower = question.lower()

    # Mots-clés qui indiquent une demande de LISTE d'associations
    list_request_patterns = [
        # Questions directes sur l'existence/disponibilité des associations
        r"(y a.t.il|est.ce qu'il y a|existe.t.il).*(association|asso|club)",
        r"(quell?e?s?).*(association|asso|club).*(sont|existent|disponible)",
        r"(liste|lister).*(association|asso|club)",

        # Questions sur les associations dans un domaine
        r"(association|asso|club).*(sport|culture|musique|théâtre|solidarité|caritatif)",
        r"(y a.t.il|existe.t.il).*(association|asso|club).*(pour|de|dans)",

        # Questions générales sur les associations disponibles
        r"(association|asso|club).*(université|campus|nanterre|disponible|proposé)",
        r"(quoi|que).*(association|asso|club).*(faire|rejoindre|adhérer)",
    ]

    # Vérifier si la question correspond à une demande de liste
    is_list_request = any(re.search(pattern, question_lower) for pattern in list_request_patterns)

    # Exclure explicitement les questions qui ne demandent PAS une liste
    exclusion_patterns = [
        r"comment (créer|fonder|monter|faire)",
        r"(créer|fonder|monter|faire).*(association|asso|club)",
        r"démarche.*pour.*(créer|fonder)",
        r"procédure.*(création|créer)",
        r"étapes.*(créer|création)",
        r"aide.*pour.*(créer|monter)",
        r"soutien.*pour.*(créer|monter)",
        r"accompagnement.*(création|créer)",
    ]

    # Si c'est une question sur la création/démarches, ne pas afficher la liste
    is_creation_question = any(re.search(pattern, question_lower) for pattern in exclusion_patterns)

    if is_creation_question:
        return None

    # Si ce n'est pas une demande de liste, ne pas utiliser cette fonction
    if not is_list_request:
        return None

    # Structure des associations par catégorie (reste inchangée)
    associations = {
        "CULTURE ARTISTIQUE ET SCIENTIFIQUE": {
            "MUSIQUE": ["Dix de Choeur", "Mélo'dix", "La Volt"],
            "THÉÂTRE": ["Compagnie P.T.D.R. (Partir Toujours De rien)", "Les Indifférents", "Les Imp'Unis"],
            "ÉLOQUENCE ET DÉBAT": ["Eloquentia Nanterre", "Lysias Paris Nanterre", "Révolte-toi Nanterre"],
            "AUDIOVISUEL/CINÉMA": ["LCC Production (ciné club)", "Nuits Noires"],
            "MÉDIAS, LECTURE ET ÉCRITURE": ["Atelier d'écriture", "Lili Bloom's Book Club", "Pile à Lire"],
            "CULTURE SCIENTIFIQUE": ["RCVA (Robot Concept Ville d'Avray)"]
        },
        "LOISIRS / ANIMATION DU CAMPUS": {
            "ANIMATION": ["CROW", "Le Cercle"],
            "VOYAGES": ["Hexa", "La Rover"]
        },
        "REPRÉSENTATION ÉTUDIANTE": [
            "L'Alternative Nanterre",
            "Fédération des Associations de l'Université de Nanterre (FAUN)",
            "Fédération des Associations de Paris Nanterre (FAX)",
            "UNEF Nanterre",
            "Union Générale Etudiante de Nanterre-FSE (UGEN FSE)"
        ],
        "SPORT": [
            "Altiski Paris Nanterre",
            "Cheerleading Paris Nanterre",
            "La Ocho Paris Nanterre",
            "Nanterre Association Voile (la NAV)"
        ],
        "SOLIDARITÉ ET ENTRAIDE": [
            "Aumônerie Catholique des étudiant·es de Nanterre (Antenne Nanterre)",
            "Épicerie Sociale et Solidaire Agoraé (tenue par l'association de filière ASEGA)",
            "Étudiants Musulmans de France (EMF Nanterre)",
            "Juris Club Paris Ouest",
            "Une Couverture Pour l'Hiver UCPH (Antenne Nanterre)",
            "Union des Étudiants Juifs de France, Section Nanterre"
        ],
        "CARITATIF": [
            "Amnesty International Groupe Jeunes 3047 (Antenne Nanterre)",
            "L'Athéna",
            "UNICEF Campus Paris Nanterre (Antenne d'UNICEF France)"
        ],
        "CULTURES DU MONDE": [
            "Amicale des Étudiants Sénégalais de Nanterre",
            "Andina",
            "Paris Nanterre Maroc"
        ],
        "CITOYENNETÉ": [
            "Cercle Marxiste de Nanterre (CMN)",
            "Le Poing Levé",
            "Meridio Nanterre",
            "Model United Nations Society Paris Nanterre (MSPN)",
            "Nanterre Décoloniale",
            "Union Etudiante Nanterre"
        ],
        "ÉCOLOGIE": [
            "Les Unis Verts Nanterre"
        ]
    }

    # Fonction pour vérifier si la question concerne une catégorie spécifique
    def is_category_question(category):
        category_keywords = {
            "CULTURE ARTISTIQUE ET SCIENTIFIQUE": ["culture", "art", "artistique", "scientifique", "musique", "théâtre",
                                                   "éloquence", "débat", "audiovisuel", "cinéma", "média", "lecture",
                                                   "écriture"],
            "LOISIRS / ANIMATION DU CAMPUS": ["loisir", "animation", "campus", "voyage"],
            "REPRÉSENTATION ÉTUDIANTE": ["représentation", "étudiant", "syndicat", "politique"],
            "SPORT": ["sport", "sportif", "sportive"],
            "SOLIDARITÉ ET ENTRAIDE": ["solidarité", "entraide", "aide", "social"],
            "CARITATIF": ["caritatif", "humanitaire", "bénévolat"],
            "CULTURES DU MONDE": ["culture", "monde", "international", "pays"],
            "CITOYENNETÉ": ["citoyen", "citoyenneté", "politique", "engagement"],
            "ÉCOLOGIE": ["écologie", "environnement", "développement durable", "vert"]
        }

        return any(keyword in question_lower for keyword in category_keywords.get(category, []))

    # Construire la réponse
    response = "# Associations étudiantes de l'Université Paris Nanterre\n\n"

    # Vérifier si la question concerne une catégorie spécifique
    specific_categories = []
    for category in associations.keys():
        if is_category_question(category):
            specific_categories.append(category)

    # Si des catégories spécifiques sont identifiées, n'inclure que celles-ci
    categories_to_include = specific_categories if specific_categories else associations.keys()

    for category in categories_to_include:
        response += f"## {category}\n\n"
        if isinstance(associations[category], dict):  # Pour les catégories avec sous-catégories
            for subcategory, assos in associations[category].items():
                response += f"### {subcategory}\n"
                for asso in assos:
                    response += f"- {asso}\n"
                response += "\n"
        else:  # Pour les catégories sans sous-catégories
            for asso in associations[category]:
                response += f"- {asso}\n"
            response += "\n"
...
def ask_ollama_improved(context, question):
    ...
    # Détection plus précise des questions sur les associations (pour la liste)
    is_association_list_question = any(re.search(pattern, question_lower) for pattern in [
        r"(y a.t.il|est.ce qu'il y a|existe.t.il).*(association|asso|club)",
        r"(quell?e?s?).*(association|asso|club).*(sont|existent|disponible)",
        r"(liste|lister).*(association|asso|club)",
        r"(association|asso|club).*(sport|culture|musique|théâtre|solidarité|caritatif)",
    ...
    # Questions générales d'aide étudiante (pas forcément liste d'associations)
    ...
    ]) and not is_association_list_question
    ...
    if is_association_list_question and not is_crous_question:
        system_prompt += """
        INSTRUCTION CRITIQUE: Cette question demande une LISTE des associations étudiantes de l'université.
    ...
    if is_student_life_question:
        system_prompt += """
        INSTRUCTION CRITIQUE: Cette question concerne l'aide à la vie étudiante en général.

        Tu dois répondre selon le contexte spécifique de la question :
        - Si c'est sur la création d'association : donner les démarches et procédures
        - Si c'est sur l'aide financière : mentionner les bourses, aides d'urgence, etc.
        - Si c'est sur l'accompagnement : parler des services d'accompagnement disponibles
        - Si c'est sur le logement : donner les infos sur les résidences, aides au logement

        NE PAS automatiquement lister toutes les associations sauf si c'est explicitement demandé.
        """
    else:
        system_prompt += """
    ...
    if is_association_list_question or "association" in question_lower or any(
...
async def find_info_for_question(question, data, urls, cached_data):
    ...
    if sport_response:
        ...
        # Vérifier d'abord si c'est une question sur les associations
